...
class DownloadList extends Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: data => {
          return (
            <div>
              <Link to={`/admin/saveDownload/${data._id}`}><Button plain={true} type="info" size="small">编辑</Button></Link>
  ...
  componentDidMount(){
    this.props.headerInit([{name: '内容'}, {name: '管理', url: '/admin/manage'}, {name: '下载列表'}]);
  ...
  shouldComponentUpdate(nextProps){
    ...
    switch(status) {
      ...
      case 'success': return !(deleted && this.props.downloadList('?' + this.searchParams.toString()));
      ...
      <div>
        <div className="list-toolbar">
          <div className="add-button">
            <Link to="/admin/saveDownload"><Button type="danger">发布下载</Button></Link>
          </div>
