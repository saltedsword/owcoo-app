...
class ProductList extends Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: data => {
          return (
            <div>
              <Link to={`/admin/saveProduct/${data._id}`}><Button plain={true} type="info" size="small">编辑</Button></Link>
  ...
  componentDidMount(){
    this.props.headerInit([{name: '内容'}, {name: '管理', url: '/admin/manage'}, {name: '产品列表'}]);
    this.props.productList(this.props.location.search);
  }
  ...
  shouldComponentUpdate(nextProps){
    ...
    switch(status) {
      ...
      case 'success': return !(deleted && this.props.productList('?' + this.searchParams.toString()));
      ...
      <div>
        <div className="list-toolbar">
          <div className="add-button">
            <Link to="/admin/saveProduct"><Button type="danger">发布产品</Button></Link>
          </div>
